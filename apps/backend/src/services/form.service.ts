    ...
    async save(demandeDTO: any, eventEmitter: EventEmitter) {
        ...
        this.fileService.generatePDF(html, function (error, stream) {
            ...
            if (demandeDTO.email != null) {
                console.log(stream)
                stream && self.emailService.addFile(config.mail.pdfFilename + ".pdf", stream);
                self.emailService.sendEmail(new Array(demandeDTO.email, process.env.EMAIL_FROM), eventEmitter);
            }
        });
    }
